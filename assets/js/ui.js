...
const PLACEHOLDER_BACKDROP =
  "https://placehold.co/300x169/141414/9b111e?text=CineFlix";
...
export function crearFilaDeSkeleton(cantidad = 8, variante = "card") {
  ...
  for (let i = 0; i < cantidad; i++) {
    ...
    skeleton.className =
      variante === "top" ? "cf-skeleton cf-skeleton-wide" : "cf-skeleton";
...
export function crearCard(contenido) {
  ...
  const subtitulo =
    contenido.tipo === "tv" ? "Serie" : contenido.generos?.[0] || "";
...
export function mostrarPantallaDeCarga(contenedor, variante = "card") {
  ...
  seccionDeCarga.innerHTML =
    '<div class="cf-row-head"><h2 class="cf-row-title">Cargando…</h2></div>';
...
export function inicializarHero(listaDestacados) {
  ...
  function irASlide(indice) {
    ...
    if (etiquetaTipo) {
      etiquetaTipo.textContent =
        contenido.tipo === "tv" ? "⬤ Serie" : "⬤ Película";
    }
    ...
    document
      .querySelectorAll(".cf-hero-dot")
      .forEach((punto, j) =>
        punto.classList.toggle("is-active", j === indiceActual),
      );
  ...
  function reiniciarTemporizador() {
    ...
    temporizadorSlideshow = setInterval(() => irASlide(indiceActual + 1), 7000);
  }
...
export function renderizarChipsDeGenero(listaDeGeneros, alSeleccionar) {
  ...
  listaDeGeneros.forEach((genero) => {
    ...
    chip.addEventListener("click", () => {
      contenedorChips
        .querySelectorAll(".cf-genre-chip")
