...
Aqui hay un error por solucionar y es que no se entinede el porque pero el idioma en es-MX, no esta funcionando,
por lo que pordefecto cada video esta en su idioma original.
...
| `obtenerPeliculasPopulares(pag)`   | Películas populares en America Latina         |
| `obtenerSeriesPopulares(pag)`      | Series populares en America Latina            |
| `obtenerPeliculasMejorValoradas()` | Películas con mayor calificación              |
| `obtenerSeriesMejorValoradas()`    | Series con mayor calificación                 |
| `obtenerEnCartelera()`             | Películas en cines en America Latina ahora    |
